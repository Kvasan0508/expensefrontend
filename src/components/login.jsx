...
const navigate = useNavigate()
  const Loginuser = async () => {
    try {
      const response = await axiosInstance.post("elite/login", {
        email: cred.email,
          ...
          <div className=" w-[90%] lg:w-[50%] p-5 lg:min-h-[400px] min-h-[40%] h-auto relative bg-[#f5f5f5]">
            <div className="w-full grid grid-cols-1 gap-4">
              <div className="grid grid-cols-1 gap-4">
                <h2 className="mb-2 text-xl text-center font-bold">Login</h2>
                <TextField
                  fullWidth
                  label="Email"
                  value={cred.email}
              ...
              <div className="grid grid-cols-1 gap-4">
                <p>new user signup here <Link to="/signup" className="font-bold">Signup</Link></p>
              </div>
            </div>
          </div>
        </div>
